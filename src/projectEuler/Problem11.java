package projectEuler;
//import java.util.*;

public class Problem11 {

	public static void main(String[] args) {
		int[] row1 = { 8, 02, 22, 97, 38, 15, 00, 40, 00, 75, 04, 05, 07, 78, 52, 12, 50, 77, 91, 8 };
		int[] row2 = { 49, 49, 99, 40, 17, 81, 18, 57, 60, 87, 17, 40, 98, 43, 69, 48, 04, 56, 62, 00 };
		int[] row3 = { 81, 49, 31, 73, 55, 79, 14, 29, 93, 71, 40, 67, 53, 88, 30, 03, 49, 13, 36, 65 };
		int[] row4 = { 52, 70, 95, 23, 04, 60, 11, 42, 69, 24, 68, 56, 01, 32, 56, 71, 37, 02, 36, 91 };
		int[] row5 = { 22, 31, 16, 71, 51, 67, 63, 89, 41, 92, 36, 54, 22, 40, 40, 28, 66, 33, 13, 80 };
		int[] row6 = { 24, 47, 32, 60, 99, 03, 45, 02, 44, 75, 33, 53, 78, 36, 84, 20, 35, 17, 12, 50 };
		int[] row7 = { 32, 98, 81, 28, 64, 23, 67, 10, 26, 38, 40, 67, 59, 54, 70, 66, 18, 38, 64, 70 };
		int[] row8 = { 67, 26, 20, 68, 02, 62, 12, 20, 95, 63, 94, 39, 63, 8, 40, 91, 66, 49, 94, 21 };
		int[] row9 = { 24, 55, 58, 05, 66, 73, 99, 26, 97, 17, 78, 78, 96, 83, 14, 88, 34, 89, 63, 72 };
		int[] row10 = { 21, 36, 23, 9, 75, 00, 76, 44, 20, 45, 35, 14, 00, 61, 33, 97, 34, 31, 33, 95 };
		int[] row11 = { 78, 17, 53, 28, 22, 75, 31, 67, 15, 94, 03, 80, 04, 62, 16, 14, 9, 53, 56, 92 };
		int[] row12 = { 16, 39, 05, 42, 96, 35, 31, 47, 55, 58, 88, 24, 00, 17, 54, 24, 36, 29, 85, 57 };
		int[] row13 = { 86, 56, 00, 48, 35, 71, 89, 07, 05, 44, 44, 37, 44, 60, 21, 58, 51, 54, 17, 58 };
		int[] row14 = { 19, 80, 81, 68, 05, 94, 47, 69, 28, 73, 92, 13, 86, 52, 17, 77, 04, 89, 55, 40 };
		int[] row15 = { 04, 52, 8, 83, 97, 35, 99, 16, 07, 97, 57, 32, 16, 26, 26, 79, 33, 27, 98, 66 };
		int[] row16 = { 88, 36, 68, 87, 57, 62, 20, 72, 03, 46, 33, 67, 46, 55, 12, 32, 63, 93, 53, 69 };
		int[] row17 = { 04, 42, 16, 73, 38, 25, 39, 11, 24, 94, 72, 18, 8, 46, 29, 32, 40, 62, 76, 36 };
		int[] row18 = { 20, 69, 36, 41, 72, 30, 23, 88, 34, 62, 99, 69, 82, 67, 59, 85, 74, 04, 36, 16 };
		int[] row19 = { 20, 73, 35, 29, 78, 31, 90, 01, 74, 31, 49, 71, 48, 86, 81, 16, 23, 57, 05, 54 };
		int[] row20 = { 01, 70, 54, 71, 83, 51, 54, 69, 16, 92, 33, 48, 61, 43, 52, 01, 89, 19, 67, 48 };
		
		int[][] grid = { row1, row2, row3, row4, row5, row6, row7, row8, row9, row10, row11, row12, row13, row14, row15, row16, row17, row18, row19, row20 };
		int greatest = 0;
		
		for (int x = 0; x < 20; x++) {
			int mark = 3;
			
			while (mark < 20) {
				
				int rowMultiplication = 1;
				int columnMultiplication = 1;
				int diagonalMultiplication = 1;
				int rowElement, columnElement, diagonalElement;
				
				for (int y = 0; y < 4; y++) {
					
					int index = mark - y;
					
					// horizontal
					rowElement = grid[x][index];
					rowMultiplication = rowMultiplication * rowElement;
					
					// vertical
					columnElement = grid[index][x];
					columnMultiplication = columnMultiplication * columnElement;
					
					if (index < 17) {
						// diagonal
						System.out.print(index + ", ");
						diagonalElement = grid[index][index];
						diagonalMultiplication = diagonalMultiplication * diagonalElement;
						
					}
				}
				System.out.println("diagonal " + diagonalMultiplication);
				//System.out.println("row " + rowMultiplication + ", column: " + columnMultiplication );
				
				if (rowMultiplication > greatest) {
					greatest = rowMultiplication;
					//System.out.println(multiplication);
				}
				if (columnMultiplication > greatest) {
					greatest = columnMultiplication;
					//System.out.println(multiplication);
				}
				mark = mark + 1;
			}
		}
		
		// go all over rows (horizontal)
		/*for (int row = 0; row < 20; row++) {
			int[] current = grid[row];
			int mark = 3;
			
			// go all over elements in a row
			while (mark < 20) {
				// take 4 elements and calculate the multiplication
				int multiplication = 1;
				for (int column = 0; column < 4; column++) {
					int element = mark - column;
					multiplication = multiplication * current[element];
				}
				// is multiplication greater than "greatest"?
				if (multiplication > greatest) {
					greatest = multiplication;
					System.out.println(multiplication);
				}
				
				mark = mark + 1;
			}	
		}
		
		// go all over columns 
		for (int column = 0; column < 20; column++) {
			int mark = 3;
			
			// go all over rows in a column
			while (mark < 20) {
				// take 4 elements and calculate the multiplication
				int multiplication = 1;
				for (int row = 0; row < 4; row++) {
					int element = grid[mark - row][column];
					multiplication = multiplication * element;
					
				}
				
				if (multiplication > greatest) {
					greatest = multiplication;
					System.out.println(multiplication);
				}
				mark = mark + 1;
			}
		}*/
		
		// go all over diagonals
		

	}

}
